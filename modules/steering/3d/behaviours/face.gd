...
func calculate_steering(agent: SteeringAgent3D, parameters: SteeringParameters3D, acceleration: TargetAccelleration):
	...
	if distance_squared < parameters.zero_linear_speed_threshold:
		...
		var orientation = atan2(to_target.x, to_target.z)
		last_angle_difference = _match_orientation(agent, parameters, acceleration, orientation)


func _match_orientation(agent: SteeringAgent3D, parameters: SteeringParameters3D, acceleration: TargetAccelleration, desired_orientation: float):
